...
void drawMesh(SceneObject sceneObj) {
    ...
    mat4 model = Translate(sceneObj.loc) * Scale(sceneObj.scale) * RotateX(sceneObj.angles[0])
	...
	float frames = (30 * (glutGet(GLUT_ELAPSED_TIME) % (4000/3))/1000.0) + 1;
    calculateAnimPose(meshes[sceneObj.meshId], scenes[sceneObj.meshId], 0, frames , boneTransforms);
